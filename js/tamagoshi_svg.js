...
// Drop-in replacement for tamagoshi.js
// Public API identical: tamaTaskDone, tamaAllDone, toggleTamagoshi,
//                       applyTamagoshiSetting, changeJobyHat
...
function pickNewHat() {
  ...
  const nonBald = ALL_HATS.filter(h => h !== currentHat && h !== 'bald');
  const pool    = nonBald.length ? nonBald : ALL_HATS.filter(h => h !== currentHat);
...
function init() {
  ...
  if (!svgEl) {
    ...
    svgEl.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:160px;pointer-events:none;overflow:visible;z-index:50;';
